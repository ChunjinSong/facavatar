...
def weighted_sampling(data, obj_mask, ray_mask, num_sample, bbox_ratio=0.9):
    ...
    # mask_samples_bbox = np.ones((obj_mask.shape[0], obj_mask.shape[1], 3)).reshape(-1, 3)
    # mask_samples_bbox[ray_mask.reshape(-1)] = data['rgb']
    # mask_samples_bbox[bbox_mask.reshape(-1)] += 0.2
    # mask_samples_bbox[select_masked_inds[index_inside]] = [0,1,0]
    # mask_samples_bbox[select_masked_inds[index_outside]] = [0,0,1]
    # mask_samples_bbox = mask_samples_bbox.reshape((obj_mask.shape[0], obj_mask.shape[1], 3))
    # path = '/ubc/cs/home/c/chunjins/chunjin_scratch/project/humannerf/temp/mask_samples_bbox.png'
    # cv2.imwrite(path, mask_samples_bbox * 255)
...
def get_patch_ray_indices(ray_mask, candidate_mask, patch_size, H, W):
    ...
    # select_inds_2d = np.where(inter_mask)
    # select_inds_2d = np.concatenate([select_inds_2d[0][:,None], select_inds_2d[1][:,None]], axis=1)

    # img_samp = full_img
    # img_samp = img_samp.reshape(-1, 3)
    # img_samp[select_masked_inds] = [0, 0, 1]
    # img_samp = img_samp.reshape(full_img.shape)
    # path = '/ubc/cs/home/c/chunjins/chunjin_scratch/project/humannerf/temp/img_samp_0.png'
    # cv2.imwrite(path, (img_samp * 255).astype(int))
    #
    # img_select = np.zeros_like(full_img)
    # img_select[y_min:y_max, x_min:x_max] = full_img[y_min:y_max, x_min:x_max]
    # path = '/ubc/cs/home/c/chunjins/chunjin_scratch/project/humannerf/temp/img_samp_0_mask.png'
    # cv2.imwrite(path, (img_select * 255).astype(int))
...
def patch_sampling(data, full_img, subject_mask, ray_mask, img_size, patch_size, N_patch, sample_subject_ratio=0.8):
    ...
    # img_samp = full_img.copy()
    # # uv_samp = output['uv']
    # rgb_samp = output['rgb']
    # img_samp = img_samp.reshape(-1, 3)
    # img_samp[select_inds_img[index_inside]] = [0,0,1]
    # img_samp[select_inds_img[index_outside]] = [0,1,0]
    # img_samp = img_samp.reshape((subject_mask.shape[0], subject_mask.shape[1], 3))
    # path = '/ubc/cs/home/c/chunjins/chunjin_scratch/project/humannerf/temp/img_samp.png'
    # cv2.imwrite(path, (img_samp * 255).astype(int))
    #
    # path = '/ubc/cs/home/c/chunjins/chunjin_scratch/project/humannerf/temp/img_targets.png'
    # img_targets += 0.2
    # cv2.imwrite(path, (img_targets * 255).astype(int))

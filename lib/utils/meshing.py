...
def generate_mesh(func, verts, type='sdf', level_set=0, res_init=32, res_up=3, point_batch=5000, device=None):
    try:
        ...
        while points.shape[0] != 0:
            ...
            points = torch.tensor(points, device=device).float()

            values = []
            for _, pnts in enumerate((torch.split(points, point_batch, dim=0))):
                out = func(pnts)
                values.append(out[type].data.cpu().numpy())
